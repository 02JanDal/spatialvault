...
/// Test that renaming a collection updates the table_name in collections and renames the DB table
#[tokio::test]
async fn test_rename_collection_updates_table_name() {
    let app = TestApp::new().await;

    // Create a vector collection
    let collection = test_collection_request("rename-before", "vector");
    let create_response = app.post_json("/collections", &collection).await;
    create_response.assert_status(StatusCode::CREATED);
    ...
    // Create a feature in the original collection
    let item = test_feature_request();
    let item_response = app
        .post_json(&format!("/collections/{}/items", collection_id), &item)
        .await;
    item_response.assert_status(StatusCode::CREATED);

    // Rename the collection by patching with a new id
    // Derive the owner prefix from the existing collection_id (everything before the last ':')
    let owner_prefix = collection_id
        .rsplit_once(':')
        .map(|(prefix, _)| prefix)
        .unwrap_or(collection_id);
    let new_id = format!("{}:rename-after", owner_prefix);
    let patch = serde_json::json!({ "id": new_id });

    let rename_response = app
        .patch_json(&format!("/collections/{}", collection_id), &patch, &etag)
        .await;
    rename_response.assert_success();

    let renamed: serde_json::Value = rename_response.json();
    assert_eq!(renamed["id"].as_str(), Some(new_id.as_str()));

    // Verify the collections table has the updated table_name
    ...
    .await
    .expect("Collection should exist with new canonical name");

    assert_eq!(row.0, "rename-after", "table_name should be updated after rename");

    // Verify features are accessible via the new collection name
    let items_response = app.get(&format!("/collections/{}/items", new_id)).await;
    items_response.assert_success();

    let items: serde_json::Value = items_response.json();
    assert_eq!(
        items["numberMatched"].as_u64().unwrap_or(0),
        1,
        "Feature should still be accessible after rename"
    );
}
