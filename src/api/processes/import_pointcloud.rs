...
/// Process definition for point cloud import
pub const PROCESS_ID: &str = "import-pointcloud";

/// Input schema for point cloud import
#[derive(Debug, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct ImportPointCloudInputs {
    /// Target collection ID (creates if doesn't exist)
    pub collection: String,

    /// Point cloud data - either inline (base64) or reference (href)
    pub data: InputValue,
    ...
    /// Whether to skip conversion if already COPC
    #[serde(default = "default_skip_if_copc")]
    pub skip_if_copc: bool,
}

fn default_skip_if_copc() -> bool {
    true
}

impl ImportPointCloudInputs {
    /// Validate the inputs
...
/// Output schema for point cloud import
#[derive(Debug, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub struct ImportPointCloudOutputs {
    /// Created item ID
    pub item_id: String,
...
pub fn process_description() -> serde_json::Value {
    serde_json::json!({
        ...
        "description": "Import a point cloud file into a collection. Accepts COPC (pass-through) or LAS/LAZ (converted to COPC via PDAL). Data can be provided inline (base64-encoded) or as a reference URL.",
        "version": "1.0.0",
        ...
        "inputs": {
            ...
            "data": {
                ...
                "description": "Point cloud file data - either inline base64-encoded content or a reference URL",
                "schema": {
                    "oneOf": [
                        {
                            "type": "object",
                            ...
                            "properties": {
                                "value": {
                                    ...
                                    "description": "Base64-encoded point cloud file content"
                                },
                                "mediaType": {
                                    "type": "string",
                                    "description": "Media type (e.g., application/vnd.laszip)"
                                }
                            }
                        },
                        {
                            ...
                            "properties": {
                                "href": {
                                    "type": "string",
                                    "format": "uri",
                                    "description": "URL to the point cloud file (S3 URI or HTTP URL)"
                                },
                                "type": {
                                    "type": "string",
            ...
            "skip_if_copc": {
                "title": "Skip if COPC",
                "description": "Skip conversion if source is already a valid COPC",
        ...
        "outputs": {
            ...
            "converted": {
                "title": "Converted",
                "description": "Whether the file was converted to COPC",
                "schema": { "type": "boolean" }
            }
        }
    })
...
mod tests {
    ...
    fn test_validate_inline_input() {
        let inputs = ImportPointCloudInputs {
            ...
            data: InputValue::Inline(InlineValue {
                ...
                media_type: Some("application/vnd.laszip".to_string()),
            }),
            title: None,
            datetime: None,
            properties: None,
    ...
    fn test_validate_reference_input() {
        let inputs = ImportPointCloudInputs {
            ...
            data: InputValue::Reference(ReferenceValue {
                href: "https://example.com/file.laz".to_string(),
                media_type: Some("application/vnd.laszip".to_string()),
            }),
            title: None,
            datetime: None,
